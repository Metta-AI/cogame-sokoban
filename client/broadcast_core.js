...
//
// Inherited from coworld-ctf. This file is the GAME-AGNOSTIC sprite-protocol
// parser and compositor: it decodes sprite definitions, object placements and
// the reserved chrome sprite, and composites them onto the canvas. The only
// game-specific reference it carried was the wire-constants global, renamed
// here (the CTF_WIRE global becomes SOKOBAN_WIRE) exactly as the design note
// specifies. Nothing else is edited, so the canvas/DPR sizing, the camera, the
// feed queue and pushFeed's SIGNATURE (the cogball 0.1.4 latch scar), banner,
// the beat and lull machinery, the endcard builder, the speed chips and the
// ?embed=1 path are byte-for-byte the starter's.
...
(function() {
  ...
  const CHROME_SPRITE_ID =
    (window.SOKOBAN_WIRE && window.SOKOBAN_WIRE.chromeSpriteId) || 4090;
  ...
  function BroadcastCore(config) {
    ...
    // in src/sokoban/sim.nim) and a packet is one such state. Drawn as-is that
