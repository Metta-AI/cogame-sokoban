...
# use-after-free fixed in sokoban_replay.nim (emscripten_exit_with_live_runtime)
...
# sokoban_error_len(). Aborting keeps linear memory intact, and the page reads
# sokoban_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "sokoban_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_sokoban_load_replay,_sokoban_frame,_sokoban_input,_sokoban_packet_ptr,_sokoban_packet_len,_sokoban_mismatch_tick,_sokoban_error_ptr,_sokoban_error_len,_sokoban_stage_ptr,_sokoban_stage_len
  """).replace("\n", " ")
)
